...
namespace Parse {
  ...
  public class ParseUser : ParseObject {
    ...
    private static object isAutoUserEnabledMutex = new object();
    private static bool autoUserEnabled;
    ...
    public Task SignUpAsync(CancellationToken cancellationToken) {
      return taskQueue.Enqueue(toAwait => SignUpAsync(toAwait, cancellationToken),
        ...
        public static async Task<ParseUser> LogInAsync(string authType, Dictionary<string, object> authData, CancellationToken token)
        {
            var result = await UserController.LogInAsync(authType, authData, token).OnSuccess(t => {
    ...
    /// if necessary, asynchronously.
    /// </summary>
    internal static Task<ParseUser> GetCurrentUserAsync(bool AutomaticUserEnabled,CancellationToken cancellationToken) {
