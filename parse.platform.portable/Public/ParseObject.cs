...
namespace Parse.Public
{
    ...
    public class ParseObject : IEnumerable<KeyValuePair<string, object>>, INotifyPropertyChanged
    {
        ...
        private readonly LinkedList<IDictionary<string, IParseFieldOperation>> _operationSetQueue =
            new LinkedList<IDictionary<string, IParseFieldOperation>>();

        private readonly IDictionary<string, object> _estimatedData = new Dictionary<string, object>();
        ...
        private static IParseObjectController ObjectController => ParseCorePlugins.Instance.ObjectController;
        ...
        {
            var className = state.ClassName ?? defaultClassName;

            var obj = (T) CreateWithoutData(className, state.ObjectId);
        ...
        {
            return TryGetValue<T>(GetFieldForPropertyName(ClassName, propertyName), out var result) ? result : defaultValue;
        }
        ...
        public void Revert()
        {
            lock (LockObject)
            {
                var wasDirty = CurrentOperations.Count > 0;
                if (!wasDirty) return;
        ...
        public void HandleFetchResult(IObjectState serverState)
        {
            lock (LockObject)
            {
            ...
            IDictionary<string, IParseFieldOperation> operationsBeforeSave)
        {
            lock (LockObject)
            {
                var opNode = _operationSetQueue.Find(operationsBeforeSave);
                var nextOperations = opNode.Next.Value;
                var wasDirty = nextOperations.Count > 0;
                _operationSetQueue.Remove(opNode);
        ...
        internal virtual void HandleSave(IObjectState serverState)
        {
            lock (LockObject)
            {
                var operationsBeforeSave = _operationSetQueue.First.Value;
                _operationSetQueue.RemoveFirst();
        ...
        protected void MergeFromServer(IObjectState serverState)
        {
        ...
        private void MergeFromObject(ParseObject other)
        {
            lock (LockObject)
            {
            ...
            if (_operationSetQueue.Count != 1)
            {
                throw new InvalidOperationException("Attempt to MergeFromObject during save.");
            }

            _operationSetQueue.Clear();
            foreach (var operationSet in other._operationSetQueue)
            {
                _operationSetQueue.AddLast(operationSet.ToDictionary(entry => entry.Key,
                    entry => entry.Value));
            }
            ...
            object root, bool traverseParseObjects, ICollection<object> seen)
        {
            ...
            if (itemsToVisit == null) yield break;
        ...
        {
            return DeepTraversal(_estimatedData)
                .OfType<ParseObject>()
                .Where(o => o.IsDirty);
        }
        ...
        {
            IDictionary<string, IParseFieldOperation> currentOperations;
            if (!IsDirty)
            {
            ...
            lock (LockObject)
            {
                ...
                deepSaveTask = DeepSaveAsync(_estimatedData, sessionToken, cancellationToken);
            }

            return deepSaveTask.OnSuccess(_ => toAwait).Unwrap().OnSuccess(_ => ObjectController.SaveAsync(State,
                currentOperations,
                sessionToken,
                cancellationToken)).Unwrap().ContinueWith(t =>
            {
            ...
            Task toAwait, CancellationToken cancellationToken)
        {
            return toAwait.OnSuccess(_ =>
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Cannot refresh an object that hasn't been saved to the server.");
                }
        ...
        private Task<ParseObject> FetchIfNeededAsyncInternal(
            Task toAwait, CancellationToken cancellationToken)
        {
        ...
        public static Task<IEnumerable<T>> FetchAllIfNeededAsync<T>(
            IEnumerable<T> objects, CancellationToken cancellationToken) where T : ParseObject
        {
            return EnqueueForAll(objects,
                toAwait => FetchAllInternalAsync(objects, false, toAwait, cancellationToken),
                cancellationToken);
        }
        ...
        {
            return EnqueueForAll(objects,
                toAwait => FetchAllInternalAsync(objects, true, toAwait, cancellationToken),
                cancellationToken);
        }
            ...
            IEnumerable<T> objects, bool force, Task toAwait, CancellationToken cancellationToken) where T : ParseObject
        {
            return toAwait.OnSuccess(_ =>
            {
                ...
                (from obj in objectsToFetch
                    group obj.ObjectId by obj.ClassName
                    into classGroup
                ...
                return Task.WhenAll(findsByClass.Values.ToList())
                    .OnSuccess(__ =>
                    {
                        if (cancellationToken.IsCancellationRequested) return objects;
        ...
        private Task DeleteAsync(Task toAwait, CancellationToken cancellationToken)
        {
            if (ObjectId == null)
            {
                return Task.FromResult(0);
            ...
            var sessionToken = ParseUser.CurrentSessionToken;

            return toAwait
                .OnSuccess(_ => ObjectController.DeleteAsync(State, sessionToken, cancellationToken))
        ...
        private static void CollectDirtyChildren(object node,
            ICollection<ParseObject> dirtyChildren,
            ...
            ICollection<ParseObject> seenNew)
        {
            foreach (var obj in DeepTraversal(node).OfType<ParseObject>())
            {
                ...
                else
                {
                    ...
                    scopedSeenNew =
                        new HashSet<ParseObject>(seenNew, new IdentityEqualityComparer<ParseObject>()) {obj};
                }
                ...
                CollectDirtyChildren(obj._estimatedData, dirtyChildren, seen, scopedSeenNew);
        ...
        private bool CanBeSerialized
        {
            get
            {
                ...
                lock (LockObject)
                {
                    return CanBeSerializedAsValue(_estimatedData);
                }
            }
        }
            ...
            Func<Task, Task<T>> taskStart, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var fullTask = taskStart(readyToStart.Task);
                ...
                Task.WhenAll(childTasks.ToArray()).ContinueWith(task => { readyToStart.SetResult(null); }, cancellationToken);
        ...
        private void RebuildEstimatedData()
        {
            lock (LockObject)
            {
                ...
                {
                    _estimatedData.Add(item);
                }

                foreach (var operations in _operationSetQueue)
                {
                    ApplyOperations(operations, _estimatedData);
                }
        ...
        internal void PerformOperation(string key, IParseFieldOperation operation)
        {
            lock (LockObject)
            {
                _estimatedData.TryGetValue(key, out var oldValue);
                var newValue = operation.Apply(oldValue, key);
                if (newValue != ParseDeleteOperation.DeleteToken)
                {
                    _estimatedData[key] = newValue;
                }
                else
                {
                ...
                var wasDirty = CurrentOperations.Count > 0;
                CurrentOperations.TryGetValue(key, out var oldOperation);
        ...
        internal void Set(string key, object value)
        {
            lock (LockObject)
            {
                ...
                {
                    throw new ArgumentException("Invalid type for value: " + value.GetType());
                }
        ...
        {
            var hasCurrent = TryGetValue<T>(key, out var current);
            if (value == null)
            {
                if (hasCurrent)
                {
        ...
        {
            AddRangeUniqueToList(key, new[] {value});
        }
        ...
        public bool ContainsKey(string key)
        {
            lock (LockObject)
            {
                return _estimatedData.ContainsKey(key);
            }
        }
        ...
        private bool CheckIsDataAvailable(string key)
        {
            lock (LockObject)
            {
                return IsDataAvailable || _estimatedData.ContainsKey(key);
            }
        }
        ...
        internal IDictionary<string, IParseFieldOperation> CurrentOperations
        {
            get
            {
                lock (LockObject)
                {
                    return _operationSetQueue.Last.Value;
                }
            }
        }
       ...
       public bool IsNew
       {
        ...
        private bool CheckIsDirty(bool considerChildren)
        {
            lock (LockObject)
            {
                return (_dirty || CurrentOperations.Count > 0 || (considerChildren && HasDirtyChildren));
            }
        }
            ...
            .GetEnumerator()
        {
            lock (LockObject)
            {
                return _estimatedData.GetEnumerator();
            }
        }
        ...
        /// </summary>
        private void OnFieldsChanged(IEnumerable<string> fieldNames)
        {
        ...
        private readonly SynchronizedEventHandler<PropertyChangedEventArgs> _propertyChanged =
