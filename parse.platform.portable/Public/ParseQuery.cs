...
namespace parse.platform.portable.Public
{
    ...
    public class ParseQuery<T> where T : ParseObject
    {
        ...
        private readonly ReadOnlyCollection<string> _orderBy;
        private readonly ReadOnlyCollection<string> _includes;
        private readonly ReadOnlyCollection<string> _selectedKeys;
        private readonly string _redirectClassNameForKey;
        private readonly int? _skip;
        private readonly int? _limit;

        internal string ClassName { get; }

        private static IParseQueryController QueryController => ParseCorePlugins.Instance.QueryController;
        ...
        private IEnumerable<string> MergeIncludes(IEnumerable<string> includes)
        {
            if (_includes == null)
            {
            ...
            var newIncludes = new HashSet<string>(_includes);
        ...
        private HashSet<string> MergeSelectedKeys(IEnumerable<String> selectedKeys)
        {
            ...
            var newSelectedKeys = new HashSet<String>(_selectedKeys);
        ...
        private IDictionary<string, object> MergeWhereClauses(IDictionary<string, object> where)
        {
            ...
            var newWhere = new Dictionary<string, object>(_where);
        ...
        /// <code>m</code> Search across multiple lines of input</param>
        /// <returns>A new query with the additional constraint.</returns>
        private ParseQuery<T> WhereMatches(string key, string pattern, string modifiers = null)
        {
        ...
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>A single ParseObject that satisfies this query, or else null.</returns>
        private Task<T> FirstOrDefaultAsync(CancellationToken cancellationToken)
        {
            EnsureNotInstallationQuery();
        ...
        {
            return _where == null ? null : _where.GetOrDefault(key, null);
        }
        ...
        internal IDictionary<string, object> BuildParameters(bool includeClassName = false)
        {
            ...
            if (_where != null)
            {
                result["where"] = PointerOrLocalIdEncoder.Instance.Encode(_where);
            }

            if (_orderBy != null)
            {
                result["order"] = string.Join(",", _orderBy.ToArray());
            }
            ...
            if (_includes != null)
            {
                result["include"] = string.Join(",", _includes.ToArray());
            }

            if (_selectedKeys != null)
            {
                result["keys"] = string.Join(",", _selectedKeys.ToArray());
            }
            ...
            if (_redirectClassNameForKey != null)
            {
                result["redirectClassNameForKey"] = _redirectClassNameForKey;
            }
        ...
        private static string GetRegexOptions(Regex regex, string modifiers)
        {
            var result = modifiers ?? "";
            if (modifiers != null && (regex.Options.HasFlag(RegexOptions.IgnoreCase) && !modifiers.Contains("i")))
            {
                result += "i";
            }

            if (modifiers != null && (regex.Options.HasFlag(RegexOptions.Multiline) && !modifiers.Contains("m")))
            {
                result += "m";
            }
        ...
        private static IDictionary<string, object> EncodeRegex(Regex regex, string modifiers)
        {
        ...
        {
            if (!(obj is ParseQuery<T>))
            {
                return false;
            }

            var other = (ParseQuery<T>) obj;
            return Equals(ClassName, other.ClassName) &&
                   _where.CollectionsEqual(other._where) &&
                   _orderBy.CollectionsEqual(other._orderBy) &&
                   _includes.CollectionsEqual(other._includes) &&
                   _selectedKeys.CollectionsEqual(other._selectedKeys) &&
                   Equals(_skip, other._skip) &&
                   Equals(_limit, other._limit);
        }
