...
namespace Parse.ParseCommon.Internal.HttpClient.Portable
{
    public class HttpClient : IHttpClient
    {
        ...
        private HttpClient(NetHttpClient client)
        {
            _client = client;
        }
                            ...
                            return resultStream.WriteAsync(buffer, 0, bytesRead, cancellationToken).OnSuccess(_ =>
                            {
                                ...
                                if (totalLength > -1)
                                {
                                    downloadProgress.Report(
                                        new ParseDownloadProgressEventArgs {Progress = 1.0 * readSoFar / totalLength});
                                }
                            });
                        }).ContinueWith(_ =>
                        {
